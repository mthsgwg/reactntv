...
export default function Feminino() {
  ...
  useEffect(() => {
    (async () => {
      const response = await api.get(
        `/youtube/v3/playlistItems?key=AIzaSyDaHQDAUz1WeUVFgxhYdbEUA-0eb2Am4Ig&part=snippet&playlistId=PLSmdHC4HiQ9JPWcjBqauuwOy0ljY6-0b3&maxResults=500`,
      );
  ...
  async function MakeNewRequest(posts) {
    setLoading(true);
    const response = await api.get(
      `/youtube/v3/playlistItems?key=${process.env.REACT_APP_API_KEY}&part=snippet&playlistId=PLSmdHC4HiQ9JPWcjBqauuwOy0ljY6-0b3&maxResults=500&pageToken=${token}`,
    );
